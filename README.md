# ClaudeE2E — Multi-Agent Boilerplate

A modern, robust workspace boilerplate designed for a **Multi-Agent System** using **Claude Code (Opus 4.6)** and **Gemini CLI (via Google Antigravity or native CLI)**.
...
The workspace is configured to support multiple AI assistants, keeping their memory, instructions, and workflows isolated but allowing them to collaborate effectively:
...
- **`.agent/`**: Infrastructure for Gemini CLI and Antigravity.
  - `rules/`: Behavior and standards for the agents.
  - `workflows/`: Executable workflows (e.g., `/slop-scan`, `/refactor-sprint`).
- **`docs/`**: Shared knowledge directory containing handoff context and communication logs between agents.
...
This boilerplate defines specialized sub-agents with strict boundaries. In Claude Code, these are distinct agents:
...
| **Researcher** | Proactively explores codebase, docs, and patterns. |
| **Bug Tracker** | Automatically captures debugging knowledge and fixes. |
...
4. **Hook Enforcement**: Agents' tool usage (like bash commands or file edits) is intercepted in Claude Code. For instance, `post-edit.sh` prevents Head of Product from editing source code, and `pre-bash.sh` forces the conventional commit format `feat(SXX): description`.

---

## 🤝 Cross-Agent Collaboration

To hand off work across agents or tools, agents can write summaries and open tasks to `docs/agent-comms/` (for example, using `docs/agent-handoff.md`). 
